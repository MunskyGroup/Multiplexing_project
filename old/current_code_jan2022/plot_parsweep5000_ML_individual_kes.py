...
acc_mat_parsweep5000 = np.load('./acc_mat_par_ke.npy')
...
for i in range(10):
    for j in range(10):
        ...
        if diff != 0:
            diff_mat[i,j] = learnable_metric(4647,7257,1011,1011,10,10, .06,.06,ke_float1[i]  ,ke_float1[j]  ,3000) 
...
ax.set_xticklabels([str(np.round(float(x),2)) for x in kes], fontdict = {'fontsize':7})
fig.colorbar(b)
