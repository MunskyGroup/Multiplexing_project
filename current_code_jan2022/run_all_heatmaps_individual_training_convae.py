...
# metadata and naming
base_dir = 'convae_run1'
data_root = '.'
model_base_name = 'convae'
...
meta_data['base_command'] = 'run_convae_commandline_2file.py'
meta_data['retrain'] = 1
        ...
        for i in range(0,n):
            ...
            for j in range(0,n):
                ...
                if set([names1[i], names1[j]]) not in pairs_already_used:
                    subprocess.run(['python', 'run_convae_commandline_2file.py',
                                    '--i=%s'%str(i),
                                    '--j=%s'%str(j),
    ...
    with tqdm.tqdm(n**2) as pbar:
        for i in range(0,n):
            for j in range(0,n):
                ...
                subprocess.run(['python', 'run_convae_commandline.py',
                                '--i=%s'%str(i),
                                '--j=%s'%str(j),
