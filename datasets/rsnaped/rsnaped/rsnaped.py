...
class Banner():
    ...
    def print_banner(self):
        if self.show == True:
            print(" \n"
                "  â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—â–‘â–‘â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—â–ˆâ–ˆâ–ˆâ•—â–‘â–‘â–ˆâ–ˆâ•—â–‘â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—â–‘â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—â–‘â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—â–‘ \n" 
                "  â–ˆâ–ˆâ•”â•â•â–ˆâ–ˆâ•—â–ˆâ–ˆâ•”â•â•â•â•â•â–ˆâ–ˆâ–ˆâ–ˆâ•—â–‘â–ˆâ–ˆâ•‘â–ˆâ–ˆâ•”â•â•â–ˆâ–ˆâ•—â–ˆâ–ˆâ•”â•â•â–ˆâ–ˆâ•—â–ˆâ–ˆâ•”â•â•â•â•â•â–ˆâ–ˆâ•”â•â•â–ˆâ–ˆâ•— \n" 
                "  â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•”â•â•šâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—â–‘â–ˆâ–ˆâ•”â–ˆâ–ˆâ•—â–ˆâ–ˆâ•‘â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•‘â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•”â•â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—â–‘â–‘â–ˆâ–ˆâ•‘â–‘â–‘â–ˆâ–ˆâ•‘ \n" 
                "  â–ˆâ–ˆâ•”â•â•â–ˆâ–ˆâ•—â–‘â•šâ•â•â•â–ˆâ–ˆâ•—â–ˆâ–ˆâ•‘â•šâ–ˆâ–ˆâ–ˆâ–ˆâ•‘â–ˆâ–ˆâ•”â•â•â–ˆâ–ˆâ•‘â–ˆâ–ˆâ•”â•â•â•â•â–‘â–ˆâ–ˆâ•”â•â•â•â–‘â–‘â–ˆâ–ˆâ•‘â–‘â–‘â–ˆâ–ˆâ•‘ \n" 
                "  â–ˆâ–ˆâ•‘â–‘â–‘â–ˆâ–ˆâ•‘â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•”â•â–ˆâ–ˆâ•‘â–‘â•šâ–ˆâ–ˆâ–ˆâ•‘â–ˆâ–ˆâ•‘â–‘â–‘â–ˆâ–ˆâ•‘â–ˆâ–ˆâ•‘â–‘â–‘â–‘â–‘â–‘â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•—â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•”â• \n" 
...
class SimulatedCell():
    ...
    dataframe_format : str, optional
        ...
        "long" format generates this dataframe: [cell_number, particle, frame, red_int_mean, green_int_mean, blue_int_mean, red_int_std, green_int_std, blue_int_std, x, y, SNR_red,SNR_green,SNR_blue].
        "short" format generates this dataframe: [cell_number, particle, frame, red_int_mean, green_int_mean, blue_int_mean, x, y].
    simulate_photobleaching : str, optional
       string to add simulated photobleaching of either 'constant' or a 'normal' distribution. if the option is constant, the average rate supplied is the constant rate. if the option is normal, mu and var are used.  The default is 'none'.
    simulated_pb_video_mu : NumPy array, optional 
        photobleaching average bleaching rates for the background video channels, A, for every channel, for the equation video*(1*exp(-A*t)). The default is np.array([0,0,0,])
    simulated_pb_spot_mu : NumPy array, optional 
        photobleaching average bleaching rates for the spot trajectories, A, for every channel, for the equation intensity_trajectories*(1*exp(-A*t)). The default is np.array([0,0,0,])       
    simulated_pb_video_var : NumPy array, optional 
        photobleaching variance rates for the background video channels, A, for every channel, for the equation video*(1*exp(-A*t)). The default is np.array([0,0,0,])
    simulated_pb_spot_var : NumPy array, optional 
        photobleaching variance rates for the spot trajectories, A, for every channel, for the equation intensity_trajectories*(1*exp(-A*t)). The default is np.array([0,0,0,]) 

    '''
    def __init__(self, base_video:np.ndarray, video_for_mask:Union[np.ndarray, None] = None,  mask_image:Union[np.ndarray, None] = None, number_spots:int = 10, number_frames:int = 20, step_size:float = 1, diffusion_coefficient:float = 0.01, simulated_trajectories_ch0:Union[np.ndarray, None]  = None, size_spot_ch0:int = 5, spot_sigma_ch0:int = 1, simulated_trajectories_ch1:Union[np.ndarray, None] = None, size_spot_ch1:int = 5, spot_sigma_ch1:int = 1, simulated_trajectories_ch2:Union[np.ndarray, None] = None, size_spot_ch2:int = 5, spot_sigma_ch2:int = 1, ignore_ch0: bool = 0, ignore_ch1: bool = 0, ignore_ch2: bool = 0, save_as_tif_uint8: bool = 0, save_as_tif: bool = 0, save_as_gif: bool = 0, save_dataframe: bool = 0, saved_file_name :str = 'temp', create_temp_folder: bool = True, intensity_calculation_method :str = 'disk_donut', using_for_multiplexing = 0, min_int_multiplexing: bool = None, max_int_multiplexing :Union[float, None] = None, perform_video_augmentation: bool = 0, frame_selection_empty_video:str = 'shuffle',ignore_trajectories_ch0:bool =0, ignore_trajectories_ch1:bool =0, ignore_trajectories_ch2:bool =0,intensity_scale_ch0:float = 10,intensity_scale_ch1:float = 10,intensity_scale_ch2:float = 10,dataframe_format:str = 'short',                simulate_photobleaching:str = 'none',
                simulated_pb_video_mu:Union[np.ndarray, None] =None,
                simulated_pb_spot_mu:Union[np.ndarray, None]  = None,
                simulated_pb_video_var:Union[np.ndarray, None] =None,
                simulated_pb_spot_var:Union[np.ndarray, None]  = None
                ):
        ...
        if simulate_photobleaching.lower() != 'none': #Patch in photobleaching - WSR 12/18/22
            if not (simulated_trajectories_ch0 is None):
                simulated_trajectories_ch0 = PhotobleachApplicationTrajectories(simulated_trajectories_ch0, simulate_photobleaching, simulated_pb_spot_mu[0], simulated_pb_spot_var[0]).apply()
            if not (simulated_trajectories_ch1 is None):
                simulated_trajectories_ch1 = PhotobleachApplicationTrajectories(simulated_trajectories_ch1, simulate_photobleaching, simulated_pb_spot_mu[1], simulated_pb_spot_var[1]).apply()
            if not (simulated_trajectories_ch2 is None):
                simulated_trajectories_ch2 = PhotobleachApplicationTrajectories(simulated_trajectories_ch2, simulate_photobleaching, simulated_pb_spot_mu[2], simulated_pb_spot_var[2]).apply()
        ...
        self.simulate_photobleaching=simulate_photobleaching
        self.simulated_pb_video_mu=simulated_pb_video_mu
        self.simulated_pb_spot_mu = simulated_pb_spot_mu
        self.simulated_pb_video_var=simulated_pb_video_var
        self.simulated_pb_spot_var = simulated_pb_spot_var
    ...
    def make_simulation (self):
        ...
        def make_simulation(base_video_selected_channel:np.ndarray, masked_video_selected_channel:np.ndarray, spot_positions_movement:np.ndarray, time_vector:np.ndarray, polygon_array, image_size:np.ndarray, size_spot:int, spot_sigma:int, simulated_trajectories, frame_selection_empty_video,ignore_trajectories,intensity_scale,
                             simulate_photobleaching, simulated_pb_video_mu, simulated_pb_video_var):
        ...
        if self.ignore_ch0 == 0:
            tensor_image_ch0 = make_simulation(self.base_video[:, :, :, 0], self.video_removed_mask[:, :, :, 0], spot_positions_movement, self.time_vector, self.polygon_array, self.image_size, self.size_spot_ch0, self.spot_sigma_ch0, self.simulated_trajectories_ch0, self.frame_selection_empty_video,self.ignore_trajectories_ch0,self.intensity_scale_ch0, self.simulate_photobleaching, self.simulated_pb_video_mu[0], self.simulated_pb_video_var[0] )
        else:
            tensor_image_ch0 = np.zeros((self.number_frames, self.image_size[0], self.image_size[1]), dtype = np.uint16)

        # Channel 1
        if self.ignore_ch1 == 0:
            tensor_image_ch1 = make_simulation(self.base_video[:, :, :, 1], self.video_removed_mask[:, :, :, 1], spot_positions_movement, self.time_vector, self.polygon_array, self.image_size, self.size_spot_ch1, self.spot_sigma_ch1, self.simulated_trajectories_ch1, self.frame_selection_empty_video,self.ignore_trajectories_ch1,self.intensity_scale_ch1, self.simulate_photobleaching, self.simulated_pb_video_mu[1], self.simulated_pb_video_var[1])
        else:
        ...
        # if self.simulate_photobleaching.lower() != 'none':
        #     tensor_video [:, :, :, 0] =  PhotobleachApplicationBaseVideo(tensor_image_ch0,self.simulate_photobleaching, self.simulated_pb_video_mu, self.simulated_pb_video_var).apply()
        #     tensor_video [:, :, :, 1] =  PhotobleachApplicationBaseVideo(tensor_image_ch1, self.simulate_photobleaching, self.simulated_pb_video_mu, self.simulated_pb_video_var).apply()
        #     tensor_video [:, :, :, 2] =  PhotobleachApplicationBaseVideo(tensor_image_ch2, self.simulate_photobleaching, self.simulated_pb_video_mu, self.simulated_pb_video_var).apply()
...
class SimulatedCellMultiplexing ():
    ...
    def __init__(self, initial_video:np.ndarray, list_gene_sequences:list, list_number_spots:list, list_target_channels_proteins:list, list_target_channels_mRNA:list, list_diffusion_coefficients:list, list_label_names:list, list_elongation_rates:list, list_initiation_rates:list, simulation_time_in_sec:float, step_size_in_sec:float, save_as_tif:bool, save_dataframe:bool, saved_file_name:str, create_temp_folder:bool, mask_image:Union[np.ndarray, None] = None, cell_number:int = 0, save_as_gif:bool = 0, perform_video_augmentation:bool = 1, frame_selection_empty_video:str = 'shuffle',spot_size:int = 5 ,intensity_scale_ch0 = 10,intensity_scale_ch1 = 10,intensity_scale_ch2 = 10,dataframe_format='short', 
                simulate_photobleaching = 'None',
                list_target_channels_mRNA_intensity = [0,0],
                simulated_RNA_intensities_method = 'constant',
                simulated_pb_video_mu = np.array([0,0,0]),
                simulated_pb_spot_mu = np.array([0,0,0]),
                simulated_pb_video_var = np.array([0,0,0]),
                simulated_pb_spot_var = np.array([0,0,0]),
                simulated_pb_k_probes_list = None):
        ...
        self.list_target_channels_mRNA_intensity = list_target_channels_mRNA_intensity
        ...
        self.simulated_pb_video_var =simulated_pb_video_var
        self.simulated_pb_spot_var = simulated_pb_spot_var
        self.simulated_pb_k_probes_list = simulated_pb_k_probes_list
    ...
    def make_simulation (self):
        ...
        def wrapper_simulated_cell (base_video, video_for_mask = None, ssa = None, rna_intensity=None, target_channel_protein = 1,target_channel_mRNA =0,  diffusion_coefficient = 0.05, step_size = self.step_size_in_sec, spot_size = self.spot_size, spot_sigma = 2, intensity_calculation_method = 'disk_donut', using_for_multiplexing = 0, min_int_multiplexing = 0 , max_int_multiplexing = 0, save_as_gif = 0, frame_selection_empty_video = self.frame_selection_empty_video,dataframe_format =self.dataframe_format, 
                                    simulate_photobleaching = self.simulate_photobleaching,
                                    simulated_pb_video_mu = self.simulated_pb_video_mu,
                                    simulated_pb_spot_mu = self.simulated_pb_spot_mu,
                                    simulated_pb_video_var = self.simulated_pb_video_var,
                                    simulated_pb_spot_var = self.simulated_pb_spot_var, ):
            ...
            tensor_video, _,DataFrame_particles_intensities = SimulatedCell( base_video = base_video, video_for_mask = video_for_mask, mask_image=self.mask_image, number_spots = number_spots_per_cell, number_frames = ssa.shape[1], step_size = step_size, diffusion_coefficient = diffusion_coefficient, 
                                                                            simulated_trajectories_ch0 = simulated_trajectories_ch0,
                                                                            size_spot_ch0 = spot_size, spot_sigma_ch0 = spot_sigma,
                                                                            ...
                                                                            simulated_pb_spot_var = simulated_pb_spot_var,).make_simulation()
            DataFrame_particles_intensities['cell_number'] = DataFrame_particles_intensities['cell_number'].replace([0], self.cell_number)
            
            
            #if simulate_photobleaching == 'constant':
                #tensor_video = PhotobleachApplicationBaseVideo(tensor_video, simulate_photobleaching, simulated_pb_video_mu, simulated_pb_video_var)
        ...
        RNA_INTENSITY_MAX_VALUE =100 # this variable defines a value of units of RNA
        for i in range(0, self.number_genes):
            ...
            simulated_trajectories_RNA= SimulateRNA(shape_output_array=(self.list_number_spots[i], int(self.simulation_time_in_sec)), 
                                                                        rna_intensity_method = self.simulated_RNA_intensities_method,
                                                                        mean_int=self.list_target_channels_mRNA_intensity[i]  ).simulate()            
            
            list_ssa.append(ssa_solution)
            list_min_ssa.append(ssa_solution.min())
            list_max_ssa.append(ssa_solution.max())
            list_rna.append(simulated_trajectories_RNA)
        # Creating the videos
        list_DataFrame_particles_intensities = []
        for i in range(0, self.number_genes):
            if i == 0 :
                tensor_video , DataFrame_particles_intensities = wrapper_simulated_cell(self.initial_video, rna_intensity=list_rna[i],
                                                                                        video_for_mask = self.initial_video, ssa = list_ssa[i], target_channel_protein = self.list_target_channels_proteins[i],target_channel_mRNA =  self.list_target_channels_mRNA[i], diffusion_coefficient = self.list_diffusion_coefficients[i], min_int_multiplexing = min(list_min_ssa) , max_int_multiplexing = max(list_max_ssa), save_as_gif = self.save_as_gif, frame_selection_empty_video = self.frame_selection_empty_video,dataframe_format =self.dataframe_format )
            else:
                tensor_video , DataFrame_particles_intensities = wrapper_simulated_cell(tensor_video, rna_intensity=list_rna[i],
                                                                                        video_for_mask = self.initial_video, ssa = list_ssa[i], target_channel_protein = self.list_target_channels_proteins[i], target_channel_mRNA = self.list_target_channels_mRNA[i] , diffusion_coefficient = self.list_diffusion_coefficients[i],
                                                                                        using_for_multiplexing = 1, min_int_multiplexing = min(list_min_ssa) , max_int_multiplexing = max(list_max_ssa), save_as_gif = self.save_as_gif, frame_selection_empty_video = 'loop',dataframe_format =self.dataframe_format, simulate_photobleaching='none' ) # notice that for the multiplexing frame_selection_empty_video has to be 'loop', because the initial video deffines the initial background image.
            list_DataFrame_particles_intensities.append(DataFrame_particles_intensities)
        
        ##apply photobleaching to the video
        if self.simulate_photobleaching.lower() != 'none':
            tensor_video [:, :, :, 0] =  PhotobleachApplicationBaseVideo(tensor_video[:, :, :, 0],self.simulate_photobleaching, self.simulated_pb_video_mu[0], self.simulated_pb_video_var[0]).apply()
            tensor_video [:, :, :, 1] =  PhotobleachApplicationBaseVideo(tensor_video[:, :, :, 1], self.simulate_photobleaching, self.simulated_pb_video_mu[1], self.simulated_pb_video_var[1]).apply()
            tensor_video [:, :, :, 2] =  PhotobleachApplicationBaseVideo(tensor_video[:, :, :, 2], self.simulate_photobleaching, self.simulated_pb_video_mu[2], self.simulated_pb_video_var[2]).apply()
            
            new_dfs = []
            mm = 0
            for df in list_DataFrame_particles_intensities:
            ## apply post hoc to the intensity trajectories NOT COMPLETELY VALID FOR NORMAL
                ntimes = int(self.simulation_time_in_sec)
                
                ntraj = int(self.list_number_spots[mm])
                
                int_r = df['red_int_mean'].values.reshape([ntraj,ntimes])
                int_g = df['green_int_mean'].values.reshape([ntraj,ntimes])
                int_b = df['blue_int_mean'].values.reshape([ntraj,ntimes])
                int_r_pb = PhotobleachApplicationTrajectories(int_r, self.simulate_photobleaching, self.simulated_pb_video_mu[0], self.simulated_pb_video_var[0]).apply()
                int_g_pb = PhotobleachApplicationTrajectories(int_g, self.simulate_photobleaching, self.simulated_pb_video_mu[1], self.simulated_pb_video_var[1]).apply()
                int_b_pb = PhotobleachApplicationTrajectories(int_b, self.simulate_photobleaching, self.simulated_pb_video_mu[2], self.simulated_pb_video_var[2]).apply()
                
                df['red_int_mean'] = int_r_pb.reshape(ntimes*ntraj)
                df['green_int_mean'] = int_g_pb.reshape(ntimes*ntraj)
                df['blue_int_mean'] = int_b_pb.reshape(ntimes*ntraj)
                new_dfs.append(df)
                mm+=1
            list_DataFrame_particles_intensities = new_dfs
...
class PhotobleachApplicationBaseVideo():
    '''
    This class applies photobleaching post-hoc to generated interpolated base video 
    
    if the normal option is used, each time point a new X by Y image is pulled from e^(-N(mu, var)*t) and 
    multiplied by the video frame pixel by pixel
    
    if the constant option is selected, each video frame is multiplied by the constant e^(-mu*t)
    
    Parameters
    
    video : NumPy array
        Array of intensities with dimensions [T, X, Y].
    photobleaching_type : str
        Type of photobleaching - "constant" or "normal" distribution.
    photobleaching_mu : float
        photobleaching rate average for the pb rate normal distribution e^(-N(mu, var)*t), if type is constant, this is the constant value instead
    photobleaching_var : float
        photobleaching rate variance for the pb rate normal distribution e^(-N(mu, var)*t), if type is constant, this is not used
    step_size : float
        time step size, default is 1.            
        
    '''
    def __init__(self, video:np.ndarray,photobleaching_type:str, photobleaching_mu:float,photobleaching_var:float,step_size:int=1,show_plot:bool=0):
        self.video = video
        self.show_plot = show_plot
        self.step_size = step_size
        self.photobleaching_mu = photobleaching_mu #converts a list to np array
        self.photobleaching_var = photobleaching_var
        self.photobleaching_type = photobleaching_type
        
        
    def apply(self): 
        time_points, height, width = self.video.shape[0], self.video.shape[1], self.video.shape[2]
        # define type of function to search
        def fun_model_exp(t_p, photobleaching_exp_constant):
            return 1 * np.exp(-photobleaching_exp_constant*t_p)
    
        # array with time points
        t_p = np.arange(0,time_points,self.step_size) 
        if self.photobleaching_type.lower() == 'constant':
            for k in range(0,time_points):    
                self.video[k,:,:] = self.video[k,:,:]*fun_model_exp(t_p[k], self.photobleaching_mu )
        if self.photobleaching_type.lower() == 'normal':
            for k in range(0,time_points):  
                
                pb_mat_frame = np.exp( -(np.random.normal(self.photobleaching_mu,self.photobleaching_var,size=(height,width)).clip(0))*t_p[k])   
                self.video[k,:,:] = np.multiply(self.video[k,:,:], pb_mat_frame)
        return self.video


class PhotobleachApplicationTrajectories():
    '''
    This class applies photobleaching post-hoc to intensity trajectories from the rSNAPsim
    
    Parameters
    
    intensity_trajectories : NumPy array
        Array of intensities with dimensions [value, T].
    photobleaching_type : str
        Type of photobleaching - "constant" or "normal" distribution.
    photobleaching_mu : float
        photobleaching rate average for the pb rate normal distribution e^(-N(mu, var)*t), if type is constant, this is the constant value instead
    ...
    '''
    def __init__(self, intensity_trajectories:np.ndarray,photobleaching_type:str, photobleaching_mu:float,photobleaching_var:float,step_size:int=1,show_plot:bool=0):
        self.intensity_trajectories = intensity_trajectories
        self.show_plot = show_plot
        self.step_size = step_size
        self.photobleaching_mu = photobleaching_mu #converts a list to np array
        self.photobleaching_var = photobleaching_var
    ...
    def apply(self): 
        _,time_points    = self.intensity_trajectories.shape[0],self.intensity_trajectories.shape[1], 
        # define type of function to search
        def fun_model_exp(t_p, photobleaching_exp_constant):
            return 1 * np.exp(-photobleaching_exp_constant*t_p)
        ...
        if self.photobleaching_type.lower() == 'constant':
            for k in range(0,time_points):
                self.intensity_trajectories[:,k] = self.intensity_trajectories[:,k]*fun_model_exp(t_p[k], self.photobleaching_mu, )
        if self.photobleaching_type.lower() == 'normal':

            pb_matrix = np.exp( -(np.random.normal(self.photobleaching_mu,self.photobleaching_var,size=(self.intensity_trajectories.shape)).clip(0))*t_p)            
            self.intensity_trajectories = np.multiply(self.intensity_trajectories, pb_matrix)

        return self.intensity_trajectories
...
class PhotobleachingCorrectionVideo():
    def __init__(self, video:np.ndarray,mask:np.ndarray,step_size:int=1, opt_iters = 10000,normalized=False,guess=None, 
                     show_plot:bool=1,):
        self.video = video
        self.show_plot = show_plot
        self.mask = mask
    ...
    def apply_photobleaching_correction(self): 
        ...
        average_ints = np.zeros([self.video.shape[0], self.video.shape[-1],]   )
        for i in range(self.video.shape[0]):
          for j in range(self.video.shape[-1]):
            average_ints[i,j] = np.mean(self.video[i,:,:,j][self.mask == 1])
        ...
        tvec = range(self.video.shape[0]) #time vector for fitting
        exponentials = np.zeros(average_ints.shape) #array for exponentials
    
        
        for i in range(self.video.shape[-1]): # for each channel
            # fit the exponential curve
            opt,cov = curve_fit(expo,tvec,average_ints[:,i],p0=guess,maxfev=self.opt_iters)
        ...
        for i in range(self.video.shape[-1]):
          for j in range(self.video.shape[0]):
            self.video[j,:,:,i] = self.video[j,:,:,i]/exponentials[j,i] 
    
        return self.video, exponentials, opt, cov
...
class PhotobleachingCorrectionSpotsOnly():
    '''
    
    Takes a consistent size intensity array of shape (ntime, ntraj)
    and will correct it individually or globally for photobleaching by 
    fitting exponential curve(s). 

    Parameters
    ----------
    int_array : ndarray
        numpy array of intensities of shape (ntimes, ntrajs).
    indiv : bool, optional
        fit an individual exponential to each trajectory or 1 global one to subtract. The default is True.
    normalized : bool, optional
        intensity array is normalized between 0 and 1. The default is False.
    guess : tuple, optional
        tuple (a,k) guess of the exponential a*e^-kt. The default is None and will guess
        based on half the first intensity values and .3 for k.
    niter : int, optional
        number of max iterations for each fit. The default is 10000.

    Returns
    -------
    ndarray
        photobleach corrected intensity array of shape (ntimes, ntraj).

    '''
    
    def __init__(self, intensity_array:np.ndarray, indiv=True, normalized=False, guess = None, opt_iters=10000):
    ...
    def apply_photobleaching_correction(self): 
        ...
        try:  #generate an inital guess
            ...
            guess = (np.mean(self.intensity_array[0])*.5, .3,)
        ...
        corrected_intensity = np.zeros(self.intensity_array.shape) #preallocate final arr
        
        tvec = range(len(self.intensity_array)) #time vector for fitting
        exponentials = np.zeros(self.intensity_array.shape) #array for exponentials
        for i in range(self.intensity_array.shape[-1]): # for each trajectory
            # fit the exponential curve
            opt,cov = curve_fit(expo,tvec,self.intensity_array[:,i],p0=guess,maxfev=self.opt_iters)
            exponentials[:,i] = expo(tvec,opt[0],opt[1])
            if self.normalized:
                exponentials[:,i] = expo(tvec,opt[0],opt[1])
            else:
                exponentials[:,i] = expo(tvec,1.0,opt[1])
            if self.indiv: #if individual correction, divide the obtained exponential
                corrected_intensity[:,i] = self.intensity_array[:,i]/exponentials[:,i] 
        if not self.indiv:
            #if global correction, divide the average obtained exponential
            corrected_intensity = self.intensity_array/(np.mean(exponentials,axis=1)[:,np.newaxis])
        return corrected_intensity, exponentials
...
def image_processing(video_tensor,provided_mask,files_dir_path_processing,
                    particle_size=14,
    ...
    list_video_paths = []
    for i in range(0,1): 
        selected_video = video_tensor # Loading the video
