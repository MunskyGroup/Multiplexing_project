...
'''
was generated backwards in kis, so we have to adjust that:

'''
acc_mat_parsweep5000 = np.load('./acc_mat_par_ke_ki.npy')
...
for i in range(10):
    for j in range(10):
        ...
        if diff != 0:
            diff_mat[i,j] = learnable_metric(4647,7257,1011,1011,10,10, ki_float1[i],ki_float2[i],ke_float1[j],ke_float2[j],3000) 
            #diff_mat[i,j] = diff
...
ax.set_yticklabels([str(np.round(float(x),2)) for x in kes][::-1], fontdict = {'fontsize':7})
